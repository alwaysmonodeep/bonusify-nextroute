...
const OrderPage = () => (
  <div className="min-h-screen bg-white my-5 mb-8 sm:mb-12 sm:pt-5 px-3 sm:px-6 lg:px-10 pb-0">
        ...
        <div className="divide-y divide-gray-100">
          {orders.length === 0 ? (
            ...
            orders.map((order, i) => {
              ...
              return (
                <div key={order.id} className={`px-4 sm:px-6 lg:px-8 py-4 sm:py-6 hover:bg-gray-50 transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                  <div className="flex items-center">
                    <div className="flex-1 flex items-center space-x-2 sm:space-x-4">
                      <div className="hidden md:block flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 bg-gray-100 rounded-full items-center justify-center overflow-hidden">
                        <Image src={order.icon} alt="Order icon" width={30} height={48} className="rounded-full object-cover" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-xs lg:text-sm text-gray-500 truncate">OID: {order.transactionId}</p>
                        <p className="text-xs lg:text-sm text-gray-400 hidden sm:block">{order.date}</p>
                      </div>
                    </div>
                    <div className="flex-1 text-center">
                      <span className="text-sm sm:text-base lg:text-lg font-semibold text-gray-900">₹{order.amount}</span>
                    </div>
                    <div className="flex-1 text-center">
                      <span className="text-sm sm:text-base lg:text-lg font-semibold text-green-600">₹{order.reward}</span>
                      <div className="flex items-center justify-center space-x-1 sm:space-x-2 mt-1">
                        <Icon className={`w-3 h-3 sm:w-4 sm:h-4 ${color}`} />
                        <span className={`font-medium text-xs sm:text-xs lg:text-sm ${color}`}>{text}</span>
                      </div>
                    </div>
                  </div>
                </div>
